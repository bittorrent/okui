...
protected:
    ...
    public:
        virtual void setAttribute(stdts::string_view name, stdts::string_view value) override {
            if (name == "font") {
                setFont(std::string(value) + ".png", std::string(value) + ".fnt");
            } else if (name == "horizontal-alignment") {
                if (value == "center") {
                    setAlignment(views::TextView::Style::HorizontalAlignment::kCenter);
                } else if (value == "left") {
                    setAlignment(views::TextView::Style::HorizontalAlignment::kLeft);
                } else if (value == "right") {
                    setAlignment(views::TextView::Style::HorizontalAlignment::kRight);
                }
            } else if (name == "vertical-alignment") {
                if (value == "middle") {
                    setAlignment(views::TextView::Style::VerticalAlignment::kMiddle);
                } else if (value == "bottom") {
                    setAlignment(views::TextView::Style::VerticalAlignment::kBottom);
                } else if (value == "top") {
                    setAlignment(views::TextView::Style::VerticalAlignment::kTop);
                }
            } else {
        ...
        virtual void setAttribute(stdts::string_view name, std::vector<ValueComponent> components) override {
            if (name == "letter-spacing") {
                setLetterSpacing(SumNumberComponents(components).value_or(0.0));
            } else if (name == "text-size") {
                setTextSize(SumNumberComponents(components).value_or(0.0));
            } else if (name == "text-color") {
                setTextColor(SumColorComponents(components).value_or(Color::kBlack));
            } else if (name == "weight") {
                setWeight(SumNumberComponents(components).value_or(100.0));
            } else {
